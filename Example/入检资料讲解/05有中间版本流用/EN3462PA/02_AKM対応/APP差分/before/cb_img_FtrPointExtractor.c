...
static slong cb_FPE_WeightedHaarLikeChkEdge_FRRR_LEFT( IMPLIB_IMGID nIntegralImg, const CB_FPE_HAAR_LIKE_FTR* const ptHaarLike, const CB_FPE_ADD_CHK_SRCH_RGN_INFO* const ptSrchRgnInfo );
static slong cb_FPE_WeightedHaarLikeChkEdge_FRRR_RIGHT( IMPLIB_IMGID nIntegralImg, const CB_FPE_HAAR_LIKE_FTR* const ptHaarLike, const CB_FPE_ADD_CHK_SRCH_RGN_INFO* const ptSrchRgnInfo );
#endif /* CALIB_PARAM_P32S_EUR_UK */
static slong cb_FPE_CalcLeftBrightness( CB_FPE_BRIGHT_CALC_INFO *ptCalcInfo );
static slong cb_FPE_CalcRightBrightness( CB_FPE_BRIGHT_CALC_INFO *ptCalcInfo );
	...
	for ( ni = CB_ADD_PATTERN_POS_LEFT; ni < CB_ADD_PATTERN_POS_MAXNUM; ni++ )
	{
		...
		*/
			...
			|| ( ( CB_ADD_PATTERN_POS_LEFT == ni ) && ( E_CB_SELFCALIB_CAMPOS_REAR == nCamDirection ) ) )
		{
			...
			nRet = cb_FPE_WeightedHaarLikeChkEdge_FRRR_LEFT( ptFPEInfo->nImg[CB_FPE_INTEGRAL_IMG_NORMAL], &(ptFPEInfo->tHaarLike[CB_FPE_HAAR_LIKE_CHK_EDGE]), &( ptSrchRgnInfo[ni].tPntImgSideChkCenterNormalSize ) );
		}
		else if ( ( ( CB_ADD_PATTERN_POS_RIGHT == ni ) && ( E_CB_SELFCALIB_CAMPOS_FRONT == nCamDirection ) ) 
			|| ( ( CB_ADD_PATTERN_POS_RIGHT == ni ) && ( E_CB_SELFCALIB_CAMPOS_REAR == nCamDirection ) ) )
		{
			...
			nRet = cb_FPE_WeightedHaarLikeChkEdge_FRRR_RIGHT( ptFPEInfo->nImg[CB_FPE_INTEGRAL_IMG_NORMAL], &(ptFPEInfo->tHaarLike[CB_FPE_HAAR_LIKE_CHK_EDGE]), &( ptSrchRgnInfo[ni].tPntImgSideChkCenterNormalSize ) );
		}
#endif /* CALIB_PARAM_P32S_EUR_UK */
		else
		{
