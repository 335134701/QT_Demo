...
void AutomationTool::init()
{
    ...
    comBean->getErrCode().insert(ui->IDEdit->objectName(),comBean->getXmlOperate()->getErrCodeType().value(ui->IDEdit->objectName()));
    comBean->getErrCode().insert(ui->RelyIDEdit->objectName(),comBean->getXmlOperate()->getErrCodeType().value(ui->RelyIDEdit->objectName()));
    comBean->getErrCode().insert(ui->ResultButton->objectName(),comBean->getXmlOperate()->getErrCodeType().value(ui->ResultButton->objectName()));
    comBean->getErrCode().insert(ui->OutputButton->objectName(),comBean->getXmlOperate()->getErrCodeType().value(ui->OutputButton->objectName()));
...
{
    connect(this,&AutomationTool::JudgeIDTypeSignal,this,&AutomationTool::JudgeIDTypeSlot);
}
/**
...
void AutomationTool::on_IDEdit_editingFinished()
{
    ...
    if(rx.indexIn(ui->IDEdit->text())==0){
        ui->IDEdit->setStyleSheet(QString(nomFontColor)); //字体相关设置
        //如果存在错误码则移除错误码
        if(!comBean->getErrCode().value(ui->IDEdit->objectName()).ID.isEmpty()){
            comBean->getErrCode().remove(ui->IDEdit->objectName());
        }
        comBean->setID(ui->IDEdit->text());
        emit JudgeIDTypeSignal(ui->IDEdit->text());
    }else{
        ui->IDEdit->setStyleSheet(QString(errFontColor)); //字体相关设置
        comBean->setID("");
        //如果错误码不存在，则添加错误码
...
void AutomationTool::on_RelyIDEdit_editingFinished()
{
    ...
    if(rx.indexIn(ui->RelyIDEdit->text())==0){
        ui->RelyIDEdit->setStyleSheet(QString(nomFontColor)); //字体相关设置
        //如果存在错误码则移除错误码
        /*if(!QString(comBean->getErrCode().value(ui->RelyIDEdit->objectName())).isEmpty()){
            comBean->getErrCode().remove(ui->RelyIDEdit->objectName());
        }*/
        comBean->setRelyID(ui->RelyIDEdit->text());
        emit JudgeIDTypeSignal(ui->RelyIDEdit->text());
    }else{
        ui->RelyIDEdit->setStyleSheet(QString(errFontColor)); //字体相关设置
        //如果错误码不存在，则添加错误码
        /*if(QString(comBean->getErrCode().value(ui->RelyIDEdit->objectName())).isEmpty()){
 ...
 */
void AutomationTool::JudgeIDTypeSlot(const QString ID)
{
    QLogHelper::instance()->LogInfo("AutomationTool->JudgeIDTypeSlot() 函数触发执行!");
    QString ret=comBean->getUiMethod()->JudgeIDType(ID);
    if((ID==comBean->getID())||(ID==comBean->getRelyID()&&comBean->getID().isEmpty())){
        comBean->setIDType(ret);
    }
    if(ID==comBean->getRelyID()&&ret!=comBean->getIDType()){
        //添加错误代码，机种和依赖机种不是同一种类型错误
    }
