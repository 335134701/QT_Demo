    ...
    case SISHDefineFileflag:
        ...
        if(!filePath.isEmpty()){
            ...
            if(siFormBean->getRelyID()->isEmpty()){
                ...
                emit ReadExcelThreadSignal(filePath,*siFormBean->getRelyID(),*siFormBean->getRelyIDType(),flag);
            }
        }
        isGoON=false;
        break;
...
/**
 * @def 通过解析量产管理表和宏定义表推算出依赖ID
 * @brief SIFormMethod::EndGetRelyIDSlot
 * @param defineList
 * @param RelyID
 */
/*
void SIFormMethod::EndGetRelyIDSlot(const QList<SI_DEFINEMESSAGE> defineList, const QString RelyID)
{
    QLogHelper::instance()->LogInfo("SIFormMethod->EndGetRelyIDSlot() 函数执行!");
}
*/
